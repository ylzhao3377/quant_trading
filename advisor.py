...
class PortfolioAdvisor:
    ...
    def __init__(self, strategy_results_file, combined_log_file, snapshot_log_file):
        ...
        # Add PST date column for display purposes
    ...
    def analyze_latest_data(self, lookback_days=5):
        """
        Analyze the latest stock data and make recommendations.
        ...
        for ticker in eligible_stocks:
            try:
                ...
                # Create a mock account to use with strategy functions
                ...
                if position_info['in_position'] and position_info['position'] > 0:
                    if strategy_type == 'trend':
                        sell_recommendation = run_trend_sell_logic(mock_account, latest_row, params)
                    elif strategy_type == 'range':
                        sell_recommendation = run_range_sell_logic(mock_account, latest_row, params)
                ...
                if strategy_type == 'trend' and not position_info['in_position']:
                    trend_strength = compute_trend_strength(latest_row, params)
                    if trend_strength > 0:
                        # Calculate position size (simplified for advisor)
                        allocation_pct = float(
                            self.strategy_results[self.strategy_results['Stock'] == ticker]['Allocation (%)'].iloc[0])
                        funds_to_use = self.total_balance * (allocation_pct / 100) * 0.9  # Use 90% of allocation
                        shares_to_buy = int(funds_to_use / latest_row['close'])
                ...
                elif strategy_type == 'range' and latest_row['close'] < latest_row['Lower']:
                    range_strength = compute_range_strength(latest_row, params)
    ...
    def generate_recommendations_report(self):
        ...
        for rec in sorted(self.recommendations, key=lambda x: x['Stock']):
            ...
            report += f"Date (PST): {pst_date.strftime('%Y-%m-%d %H:%M:%S %Z')}\n"
            report += "-" * 40 + "\n"
    ...
    def update_logs(self):
        ...
        # Add PST date column
        new_log_entries['Date_PST'] = new_log_entries['Date'].apply(convert_to_pst)
    ...
    def send_recommendations_email(self, email_config):
        ...
        # Format PST dates as strings
        combined_log['Date_PST'] = combined_log['Date_PST'].dt.strftime('%Y-%m-%d %H:%M:%S %Z')
        snapshot_log['Date_PST'] = snapshot_log['Date_PST'].dt.strftime('%Y-%m-%d %H:%M:%S %Z')
